class Admin::DishesController < Admin::BaseController
  before_action :set_dish, except: %i[index new create]
  ...
  def update
    if @dish.update(dish_params)
      lash[:success] = 'Dish was successfully updated.'
      redirect_to admin_category_dishes_path(@category)
    else
  ...
  def dish_params
    params.require(:dish).permit(:name, :money, :category_id, images_attributes: %i[link id])
  end
